    ...
    if (this.regExp === null) {
      ...
      if (modText !== null ) {
        ...
        for (let i = 0; i < modRegExpLines.length; i++) {
          ...
          let modRegExpSplit = Helpers.escapeRegExpString(modRegExpLines[i]).split("#");
  ...
  getValueAvg(index) {
    ...
    if (typeof index === "undefined") {
      for (let i in this.values) {
        result += this.getValueAvg(i);
        resultCount++;
      }
    } else {      
      for (let v = 0; v < this.values[index].length; v++) {
        result += parseFloat(this.values[index][v]);
        resultCount++;
      }
    }
    return result;
  }
  isMaxRoll(allowSingleStatMaxed) {
    if (this.values === null) {
      return false;
    }
    if (typeof allowSingleStatMaxed === "undefined") {
      allowSingleStatMaxed = true;
    }
    let limits = this.modBase['trade limits'];
    let statCount = 0;
    let maxCount = 0;
    for (let l = 0; l < limits.length; l++) {
      ...
      for (let i = 0; i < limits[l].length; i++) {
        statCount++;
        if (parseFloat(limits[l][i][1]) == this.values[l][i]) {
          maxCount++;
        }
      }
    }
    ...
    return (allowSingleStatMaxed ? maxCount > 0 : maxCount === statCount);
  }
  hasTag(tag) {
    if (this.modBase === null) {
      return false;
